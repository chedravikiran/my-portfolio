...
const ProjectDetails = ({title,
  ...
  justify-center w-full h-full overflow-hidden backdrop-blur-sm'>
   ...
   >
    ...
    <img src="src/assets/close.svg" alt="" className='w-6 h-6'/>
    </button>
    ...
    <div className='p-5'>
      ...
      <div className='flex items-center justify-between mt-4'>
        <div className='flex gap-3'>
          {tags.map((tag)=>(
          <img src={tag.path}alt={tag.name} key={tag.id}
          className='rounded-lg size-10 hover-animation'/>
        ))}</div>
        <a href={href} className='inline-flex items-center gap-1 font-medium
        hover-animation cursor-pointer'>View Project <img src="src/assets/arrow-up.svg"
        className='size-4'/> </a>
      </div>
    </div>
   </motion.div>
